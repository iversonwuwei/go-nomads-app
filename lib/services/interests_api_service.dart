...
class InterestsApiService {
  ...
  Future<List<InterestsByCategory>> getInterestsByCategory() async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        return data
            .map((json) =>
                InterestsByCategory.fromJson(json as Map<String, dynamic>))
            .toList();
      }
  ...
  /// 添加当前用户兴趣
  Future<UserInterest> addCurrentUserInterest(
      AddUserInterestRequest request) async {
    try {
      final response =
          await _httpService.post('/interests/me', data: request.toJson());
  ...
  /// 批量添加用户兴趣
  Future<List<UserInterest>> addUserInterestsBatch(
      List<AddUserInterestRequest> interests) async {
    try {
      final data = interests.map((i) => i.toJson()).toList();
      final response =
          await _httpService.post('/interests/users/me/batch', data: data);
  ...
  Future<UserInterest> updateUserInterest(
      String userId, String interestId, AddUserInterestRequest request) async {
    try {
