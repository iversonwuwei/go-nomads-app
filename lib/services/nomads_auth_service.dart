    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (loginResponse.success && loginResponse.data != null) {
          // 登录成功，保存 token
          print('🎉 登录成功！');
          print('   用户: ${loginResponse.data!.user.name}');
          print('   Token: ${loginResponse.data!.accessToken.substring(0, 20)}...');

          // 设置 HttpService 的认证 token
          _httpService.setAuthToken(loginResponse.data!.accessToken);

          // 持久化保存 token 到 SQLite
          await _saveTokenToDatabase(loginResponse.data!);
  ...
  Future<bool> refreshToken([String? userId]) async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (loginResponse.success && loginResponse.data != null) {
          print('🎉 Token 刷新成功！');
          print('   新 Token: ${loginResponse.data!.accessToken.substring(0, 20)}...');
          ...
          print('❌ Token 刷新失败: ${loginResponse.message}');
          return false;
        }
      } else {
